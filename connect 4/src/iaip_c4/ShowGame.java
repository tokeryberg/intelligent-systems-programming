...
public class ShowGame {
	...
	public static void main(String[] args) {
		...
		if (args.length >= 2) {
			try {
				...
				errMsg = "Your GameInstance constructor threw an exception: "
						+ ite.toString();
				err = true;
			}
	...
	public static void printHelp(String errMsg) {
		...
		System.err
				.println("Usage: java ShowGame GameLogic1 GameLogic2 [cols rows]");
		System.err.println("GameLogic{1,2} may be:");
		System.err.println("\t" + HUMAN_CMD
				+ "\t\t - Indicates a human will be playing.");
		System.err
				.println("\tGameLogic\t - Specifies a GameLogic class extending IGameLogic to use as a competitor.");
		System.err
				.println("\tcols/rows\t - Must be integers greater than 0 and default to 7 and 6 respectively.");
	}

	/*
	 * @param cmdParam String from the command line that is either HUMAN_CMD or
	 * a path to a java class inheriting IGameLogic
	 * 
	 * @throws TBD
	 * 
	...
	public static IGameLogic parseGameLogicParam(String cmdParam)
		...
		if (!cmdParam.equalsIgnoreCase(HUMAN_CMD)) {
			retGL = (IGameLogic) Class.forName(cmdParam).getConstructor()
					.newInstance();
		}
		return retGL;
	}
